...
class LabelWoeIv(val label: Array[_], val woeIvGroup: Option[WoeIvGroup]) extends Serializable {
  // 是否可以转换为woe
  val hasWoe: Boolean = woeIvGroup.isDefined

  // 非null Woe
  lazy val splitWoeIvs: Array[WoeIv] = woeIvGroup.get.woeIvs.filter(_.position != null).toArray

  // 是否有null
  val hasNullWoe: Boolean = hasWoe && !woeIvGroup.get.woeIvs.filter(_.position == null).isEmpty

  lazy val nullWoeIv: WoeIv = woeIvGroup.get.woeIvs.filter(_.position == null).head
...
class WoeIvGroup(val woeIvs: List[WoeIv]) extends Serializable {
  ...
  /**
    * 分bin总iv
    */
  val iv: Double = woeIvs.map(_.iv).sum
  ...
  override def toString: String = s"iv: $iv, woeIvs:$woeIvs"
}
...
class SplitBinStrategy(val positions: List[List[Int]], val splitsStat: Map[Int, BinStat]) extends BinStrategy {
  ...
  lazy val splitKeys: Set[Int] = splitsStat.keySet
